...
import { WorkflowFile, ValidationIssue, WorkflowValidation } from '../types';
...
export const WorkflowPreview: React.FC<WorkflowPreviewProps> = ({
  ...
  const validateWorkflow = async (yamlContent: string) => {
    try {
  ...
  const syntaxHighlight = (yamlContent: string): string => {
    return yamlContent
      ...
      .replace(/^(\s*)(on|push|pull_request|schedule|workflow_dispatch|jobs|steps|runs-on|uses|with|env|if|needs):/gm, 
               '$1<span class="yaml-keyword">$2</span>:')
      // String values
      .replace(/:\s*(['"]).+?\1/g, (match) => 
               match.replace(/(['"]).+?\1/, '<span class="yaml-string">$&</span>'))
      // Unquoted string values
      .replace(/:\s*([^#\n\r]+?)(\s*#|$)/g, (match, value, ending) => 
               `: <span class="yaml-value">${value.trim()}</span>${ending}`)
      // Comments
      ...
      <div className="preview-header">
        ...
        <div className="validation-status">
          {validation.isValid ? (
            ...
            <span className="status-invalid">❌ {validation.issues.filter(i => i.severity === 'error').length} errors</span>
          )}
          {validation.issues.filter(i => i.severity === 'warning').length > 0 && (
            <span className="status-warnings">⚠️ {validation.issues.filter(i => i.severity === 'warning').length} warnings</span>
          )}
        </div>
      </div>
