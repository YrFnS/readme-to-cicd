...
const ConfigurationPanel = ({ configuration, onConfigurationChange }) => {
    ...
    const handleDeploymentChange = (platform) => {
    ...
    return (<div className="configuration-panel">
      ...
      <div className="config-section">
        ...
        <div className="framework-list">
          {availableFrameworks.map(framework => (<label key={framework.id} className="framework-item">
              <input type="checkbox" checked={configuration.frameworks.includes(framework.id)} onChange={(e) => handleFrameworkToggle(framework.id, e.target.checked)}/>
      ...
      <div className="config-section">
        ...
        <div className="workflow-type-list">
          {workflowTypes.map(workflowType => (<label key={workflowType.id} className="workflow-type-item">
              <input type="checkbox" checked={configuration.workflowTypes.includes(workflowType.id)} onChange={(e) => handleWorkflowTypeToggle(workflowType.id, e.target.checked)}/>
      ...
      <div className="config-section">
        ...
        <div className="deployment-list">
          {deploymentPlatforms.map(platform => (<label key={platform.id} className="deployment-item">
              <input type="radio" name="deployment" value={platform.id} checked={configuration.deploymentPlatform === platform.id} onChange={() => handleDeploymentChange(platform.id)}/>
      ...
      <div className="config-actions">
        <button className="config-button reset" onClick={() => onConfigurationChange({
            frameworks: [],
            workflowTypes: ['ci'],
            deploymentPlatform: '',
            deploymentConfig: {}
