...
export const WorkflowTabs: React.FC<WorkflowTabsProps> = ({
  ...
  if (workflows.length === 0) {
    return (
      <div className="workflow-tabs empty">
    ...
    <div className="workflow-tabs">
      ...
      <div className="tabs-container">
        {workflows.map((workflow) => (
          <div
            key={workflow.name}
            className={`workflow-tab ${selectedWorkflow === workflow.name ? 'active' : ''}`}
            onClick={() => onWorkflowSelect(workflow.name)}
          >
      ...
      <div className="tabs-summary">
        ...
        <div className="workflow-types">
          {Array.from(new Set(workflows.map(w => getWorkflowType(w.name)))).map(type => (
