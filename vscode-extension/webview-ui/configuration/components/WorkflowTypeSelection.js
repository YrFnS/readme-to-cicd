...
exports.WorkflowTypeSelection = void 0;
...
const WorkflowTypeSelection = ({ selectedTypes, onTypeToggle }) => {
    const renderWorkflowType = (workflowType) => {
        const isSelected = selectedTypes.includes(workflowType.id);
        return (<div key={workflowType.id} className={`workflow-type-item ${isSelected ? 'selected' : ''}`}>
        <label className="workflow-type-label">
          <input type="checkbox" checked={isSelected} onChange={(e) => onTypeToggle(workflowType.id, e.target.checked)}/>
          <div className="workflow-type-content">
            <div className="workflow-type-header">
              ...
              {workflowType.id === 'ci' && (<span className="recommended-badge">Recommended</span>)}
            </div>
            <p className="workflow-type-description">{workflowType.description}</p>
            
            {isSelected && (<div className="workflow-type-features">
                {getWorkflowFeatures(workflowType.id).map((feature, index) => (<span key={index} className="feature-tag">{feature}</span>))}
              </div>)}
          </div>
        </label>
      </div>);
    ...
    return (<section className="workflow-type-selection">
      ...
      {selectedTypes.length === 0 && (<div className="empty-selection">
          <p>⚠️ No workflow types selected. At least one workflow type is required.</p>
        </div>)}

      {selectedTypes.length > 0 && (<div className="selection-summary">
          <h4>Selected Workflows:</h4>
          <div className="selected-types">
            {selectedTypes.map(typeId => {
                const workflowType = WORKFLOW_TYPES.find(wt => wt.id === typeId);
                return workflowType ? (<span key={typeId} className="selected-type-badge">
...
exports.WorkflowTypeSelection = WorkflowTypeSelection;
//# sourceMappingURL=WorkflowTypeSelection.js.map