...
import { LoggingService, LogLevel, LoggingOptions } from '../../src/core/LoggingService';
...
suite('LoggingService Tests', () => {
  ...
  setup(() => {
    ...
    sandbox.stub(fs, 'statSync').returns({ mtime: new Date(), size: 1024 } as any);
  ...
  suite('Log Levels', () => {
    test('should respect log level configuration - ERROR only', () => {
      ...
      const service = new LoggingService(mockContext, mockConfiguration);
    ...
    test('should respect log level configuration - WARN and above', () => {
      ...
      assert.ok(logs.some(log => log.level === LogLevel.ERROR));
      assert.ok(logs.some(log => log.level === LogLevel.WARN));
    ...
    test('should respect log level configuration - INFO and above', () => {
      ...
      assert.ok(!logs.some(log => log.level === LogLevel.DEBUG));
  ...
  suite('Operation Logging', () => {
    ...
    test('should handle operation failures', () => {
      ...
      assert.ok(endLog);
      assert.strictEqual(endLog.level, LogLevel.ERROR);
  ...
  suite('Log Filtering', () => {
    ...
    test('should filter logs by level', () => {
      ...
      assert.ok(warnAndAbove.every(log => log.level <= LogLevel.WARN));
    });
    ...
    test('should filter logs by time range', () => {
      ...
      assert.ok(recentLogs.every(log => 
    ...
    test('should combine multiple filters', () => {
      ...
      assert.ok(filteredLogs.every(log => 
        log.level <= LogLevel.WARN && log.component === 'comp1'
      ));
    });
  });
  ...
  suite('Log Display and Export', () => {
    test('should show logs in output channel', () => {
      const outputChannel = (loggingService as any).outputChannel;
    ...
    test('should export filtered logs', async () => {
      ...
      const exportData = await loggingService.exportLogs({ level: LogLevel.ERROR });
      const parsed = JSON.parse(exportData);

      assert.strictEqual(parsed.logs.length, 1);
      assert.strictEqual(parsed.logs[0].level, LogLevel.ERROR);
    });
  });
