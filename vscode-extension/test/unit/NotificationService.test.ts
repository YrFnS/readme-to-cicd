...
import { NotificationService, NotificationOptions } from '../../src/core/NotificationService';
...
suite('NotificationService Tests', () => {
  ...
  suite('Notification Options', () => {
    ...
    test('should handle status bar notifications', async () => {
      const statusBarItem = (notificationService as any).statusBarItem;
  ...
  suite('Notification Level Filtering', () => {
    test('should respect notification level settings - none', async () => {
      ...
      const service = new NotificationService(mockContext, mockConfiguration);
  ...
  suite('Notification History', () => {
    ...
    test('should clear old notification history', async () => {
      ...
      // Mock old timestamp
      const history = (notificationService as any).notificationHistory;
      if (history.length > 0) {
  ...
  suite('Notification Summary', () => {
    test('should show notification summary', async () => {
      ...
      const summaryCall = showInfoStub.getCalls().find(call => 
        call.args[0].includes('Notification Summary')
      );
  ...
  suite('Status Bar Integration', () => {
    ...
    test('should handle persistent notifications', async () => {
      ...
      assert.ok(createStatusBarStub.calledOnce);
      const persistentNotifications = (notificationService as any).persistentNotifications;
