...
suite('Comprehensive Error Handling Integration Tests', () => {
  ...
  setup(() => {
    ...
    extensionManager = new ExtensionManager(mockContext, mockConfiguration);
  });
  ...
  suite('Error Reporting and Recovery', () => {
    ...
    test('should handle critical errors with user interaction', async () => {
      ...
      const showSaveDialogStub = sandbox.stub(vscode.window, 'showSaveDialog').resolves(
        vscode.Uri.file('/test/debug-package.json')
      );
  ...
  suite('Logging and Debugging', () => {
    ...
    test('should export debug package to file', async () => {
      const showSaveDialogStub = sandbox.stub(vscode.window, 'showSaveDialog').resolves(
        vscode.Uri.file('/test/debug-export.json')
      );
      const writeFileStub = sandbox.stub(vscode.workspace.fs, 'writeFile').resolves();
      const showSuccessStub = sandbox.stub(vscode.window, 'showInformationMessage').resolves();
      ...
      if (showSaveDialogStub.returnValues[0]) {
        await vscode.workspace.fs.writeFile(
          showSaveDialogStub.returnValues[0] as vscode.Uri,
          Buffer.from(debugData)
        );
      }
  ...
  suite('Health Check Integration', () => {
    ...
    test('should show health check results when issues found', async () => {
      ...
      await notificationService.showWarning(
        `Extension health check found ${mockHealthCheck.failedChecks} issue(s)`,
        [
          {
            title: 'View Details',
            callback: async () => {
  ...
  suite('Configuration Integration', () => {
    test('should respect configuration changes', async () => {
      ...
      const restrictiveConfig: ExtensionConfiguration = {
        debugMode: false,
      ...
      const restrictiveManager = new ExtensionManager(mockContext, restrictiveConfig);
      ...
      for (let i = 0; i < 100; i++) {
        ...
        if (i % 10 === 0) {
          await errorHandler.handleError(
            ...
            { component: 'bulk-test', operation: 'memory-test' },
            false
          );
        }
    ...
    test('should coordinate between all error handling services', async () => {
      ...
      await extensionManager.getProgressManager().withProgress(
        ...
        async (progress) => {
          ...
          await extensionManager.getErrorHandler().handleError(
            new Error('Coordinated error'),
            {
              component: 'integration',
              operation: 'coordination-test',
