...
suite('Packaging and Deployment Integration Tests', () => {
  ...
  suiteSetup(async () => {
    ...
    // Initialize services
    loggingService = LoggingService.getInstance(context);
    notificationService = NotificationService.getInstance(context, loggingService);
    versionManager = VersionManager.getInstance(context, loggingService, notificationService);
    telemetryService = TelemetryService.getInstance(context, loggingService);
  });
  ...
  suite('Telemetry Service', () => {
    ...
    test('should track workflow generation', () => {
      telemetryService.trackWorkflowGeneration(
        ['nodejs', 'typescript'],
        ['ci', 'cd'],
        true,
        5000
