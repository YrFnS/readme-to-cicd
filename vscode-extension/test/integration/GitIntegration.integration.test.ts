...
import { GitIntegration, GitRepositoryStatus } from '../../src/core/GitIntegration';
...
suite('GitIntegration Integration Tests', () => {
  ...
  async function initializeTestRepository(repoPath: string): Promise<void> {
    try {
      // Initialize Git repository
...
`);
  ...
  test('should detect non-Git directory correctly', async function() {
    ...
    const nonGitWorkspace: vscode.WorkspaceFolder = {
