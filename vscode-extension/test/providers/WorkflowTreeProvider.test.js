...
suite('WorkflowTreeProvider Tests', () => {
    ...
    setup(() => {
        ...
        mockWorkspaceManager = sinon.createStubInstance(WorkspaceManager_1.WorkspaceManager);
        ...
        treeProvider = new WorkflowTreeProvider_1.WorkflowTreeProvider(mockContext, mockWorkspaceManager, mockCLIIntegration);
    });
    teardown(() => {
        sinon.restore();
    ...
    suite('Root Items Generation', () => {
        test('should show loading state initially', async () => {
            ...
            mockCLIIntegration.executeFrameworkDetection.returns(new Promise(resolve => setTimeout(() => resolve({
                success: true,
                generatedFiles: [],
                errors: [],
                warnings: [],
        ...
        test('should show empty state when no workflows or README files', async () => {
            ...
            const children = await treeProvider.getChildren();
            assert.ok(children.some(item => item.label === 'No workflows or README files found'));
        });
        test('should show README files section when README files exist', async () => {
            ...
            await new Promise(resolve => setTimeout(resolve, 50));
            const children = await treeProvider.getChildren();
            const readmeSection = children.find(item => item.label === 'README Files (2)');
    ...
    suite('Framework Detection Integration', () => {
        test('should display detected frameworks in tree', async () => {
            ...
            await new Promise(resolve => setTimeout(resolve, 50));
            const children = await treeProvider.getChildren();
            const frameworksSection = children.find(item => item.label === 'Detected Frameworks (2)');
        ...
        test('should handle framework detection errors gracefully', async () => {
            ...
            const frameworksSection = children.find(item => item.label.includes('Detected Frameworks'));
            assert.ok(!frameworksSection);
        });
    });
  ...
  test:
    ...
    steps:
      ...
      `));
            ...
            await new Promise(resolve => setTimeout(resolve, 50));
            const children = await treeProvider.getChildren();
            const workflowsSection = children.find(item => item.label === 'Workflows (3)');
        ...
        test('should handle missing workflows directory gracefully', async () => {
            ...
            const workflowsSection = children.find(item => item.label.includes('Workflows'));
            assert.ok(!workflowsSection);
            mockReadDirectory.restore();
        });
    });
    ...
    suite('Error Handling and Edge Cases', () => {
        ...
        test('should dispose resources properly', () => {
            ...
            treeProvider._onDidChangeTreeData.dispose = disposeSpy;
            treeProvider.dispose();
            assert.ok(disposeSpy.called);
        });
    });
});
//# sourceMappingURL=WorkflowTreeProvider.test.js.map