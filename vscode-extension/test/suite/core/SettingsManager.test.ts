...
suite('SettingsManager Tests', () => {
  ...
  test('SettingsManager should initialize with default configuration', async () => {
    ...
    sinon.stub(vscode.workspace, 'onDidChangeConfiguration').returns({ dispose: sinon.stub() } as any);
  ...
  test('SettingsManager should handle configuration changes', async () => {
    let configChangeCallback: ((event: vscode.ConfigurationChangeEvent) => void) | undefined;
    ...
    if (configChangeCallback) {
      await configChangeCallback(mockEvent as any);
    }
  ...
  suite('Organization Policies', () => {
    test('should load organization policies from workspace configuration', async () => {
      const organizationPolicies: OrganizationPolicies = {
    ...
    test('should validate configuration against organization policies', async () => {
      ...
      const workflowTypeError = validation.errors.find(e => 
        e.key === 'preferredWorkflowTypes' && e.message.includes('security')
      );
      assert.ok(workflowTypeError, 'Should have missing required workflow type error');
      
      const customTemplateError = validation.errors.find(e => 
        e.key === 'customTemplates' && e.message.includes('prohibits')
      );
    ...
    test('should validate configuration changes against policies', async () => {
      ...
      // Test valid change
      const validChange = await settingsManager.validateConfigurationChange(
      ...
      const invalidChange = await settingsManager.validateConfigurationChange(
        'customTemplates', 
        ['custom.yml'], 
        'workspace'
      );
